...
这几条语句部分的美丽，来自于履行先前的理念，像是约定优与配置。当我们使用 `belongs_to :account` 是，我们假设 `projects` 表有一个叫做 `account_id` 的外键存在。当我们把 `participants` 的关联类用 `class_name` 指定为 `Person` 时，我们只需要定义 `Person` 类就可以了。这条语句便可推导出外键以及其它需要的设定。
